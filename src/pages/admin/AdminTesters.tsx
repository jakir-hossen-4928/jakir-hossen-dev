...
export const AdminTesters: React.FC<AdminTestersProps> = ({ exportTesters }) => {
    ...
    const filteredTesters = testers.filter(tester =>
        tester.displayName.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            <CardHeader className="pb-6 md:pb-8 border-b border-white/5 flex flex-col md:flex-row items-center justify-between bg-white/[0.02] px-6 md:px-8 gap-4">
                <div className="flex flex-col md:flex-row items-start md:items-center gap-4 w-full md:w-auto">
                    <div className="space-y-1 text-left">
                        ...
                        <CardDescription className="text-xs text-muted-foreground/70">Managing {filteredTesters.length} verified users.</CardDescription>
                    </div>
                ...
                <Button variant="outline" size="sm" onClick={exportTesters} className="rounded-xl border-white/10 bg-white/5 hover:bg-white/10 font-black text-[10px] md:text-xs h-9 md:h-10 px-3 md:px-4 shrink-0 uppercase tracking-widest transition-all w-full md:w-auto">
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {filteredTesters.length === 0 ? (
                                    <TableRow><TableCell colSpan={4} className="text-center py-24 text-muted-foreground/20"><Users size={48} className="mx-auto mb-4 opacity-50" /><p className="font-black uppercase tracking-widest text-[10px]">No testers found</p></TableCell></TableRow>
                                ) : (
